...
export default function SubirArchivoPage() {
  ...
  async function processProduccion(wb: XLSX.WorkBook) {
    ...
    // Obtener cajas_por_pallet del catálogo para filas donde pallets = 0 pero hay cajas
    const codigos = [...new Set(rawRows.map(r => r.codigo).filter(Boolean))];
    ...
    if (codigos.length > 0) {
      ...
      if (Object.keys(catPalletsMap).length > 0)
        addLog(`✓ Catálogo: ${Object.keys(catPalletsMap).length} productos con cajas_por_pallet`);
    }
    ...
    const inserts = rawRows.map(r => {
      ...
      const cpp = r.codigo ? (catPalletsMap[r.codigo] ?? 0) : 0;
      const effectivePallets = r.pallets > 0
        ? r.pallets
        : r.cajas > 0 && cpp > 0
          ? Math.ceil(r.cajas / cpp)
          : r.cajas > 0 ? 1 : 0;
      const invPe  = (ocupadoMap[locKey] ?? 0) + effectivePallets;
      return {
